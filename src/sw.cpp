...
void StillingerWeberRadialBasis::AddPenaltyGrad(const double weigh, double& loss, Array1D* loss_grad)
{
    ...
    int rad_size_n_types2 = p_RadialBasis->size*n_at_types2;
...
void StillingerWeberRadialBasis::AccumulateEnergy2body(Configuration& cfg, const Neighborhood& nbh) {
    ...
    for (int j = 0; j < nbh.count; j++) {
        ...
        for (int alpha = 0; alpha < p_RadialBasis->size; alpha++) {
            const int idx = alpha + nbh.types[j] * p_RadialBasis->size
                    + nbh.my_type * p_RadialBasis->size * n_at_types;
            const double val = p_RadialBasis->vals[alpha];
...
void StillingerWeberRadialBasis::AccumulateEFS2body(Configuration& cfg, const Neighborhood& nbh, const int my_ind) {
    ...
    for (int j = 0; j < nbh.count; j++) {
        ...
        for (int alpha = 0; alpha < p_RadialBasis->size; alpha++) {
            const int idx_curr_j = alpha + idx_j * p_RadialBasis->size;
            const double val = p_RadialBasis->vals[alpha];
            const double der = p_RadialBasis->ders[alpha];
...
const int my_ind) {
    ...
    for (int j = 0; j < nbh.count; j++) {
        ...
        p_RadialBasis->CalcDers(dist_j);
        for (int alpha = 0; alpha < p_RadialBasis->size; alpha++) {
            rad_bas_val_j[alpha] = p_RadialBasis->vals[alpha];
            rad_bas_der_j[alpha] = p_RadialBasis->ders[alpha];
        }
        ...
        for (int k = j+1; k < nbh.count; k++) {     
            ...
            for (int gamma = 0; gamma < p_RadialBasis->size; gamma++) {
                int idx_curr_k = gamma + idx_k * p_RadialBasis->size;
                double coeff_times_val_k = tripple_potential_coeff[idx_curr_k] * p_RadialBasis->vals[gamma];
                double coeff_times_der_k = tripple_potential_coeff[idx_curr_k] * p_RadialBasis->ders[gamma];
            ...
            for (int alpha = 0; alpha < p_RadialBasis->size; alpha++) {
                int idx_curr_j = alpha + idx_j * p_RadialBasis->size;
        ...
        for (int k = j+1; k < nbh.count; k++) {     
            ...
            for (int alpha = 0; alpha < p_RadialBasis->size; alpha++) {
                ...
                for (int gamma = 0; gamma < p_RadialBasis->size; gamma++) {
                    ...
                    double coeff_val_j_inner_coeffs_vals_k_grad_k = coeff_times_val_j * p_RadialBasis->vals[gamma];
                    double coeff_der_j_inner_coeffs_vals_k_grad_k = coeff_times_der_j * p_RadialBasis->vals[gamma];
                    double coeff_val_j_inner_coeffs_ders_k_grad_k = coeff_times_val_j * p_RadialBasis->ders[gamma];
...
void StillingerWeberRadialBasis::CalcE(Configuration& cfg) {
    ...
    for (Neighborhood& nbh : cfg.nbhs) {
        //const Neighborhood& nbh = cfg.nbhs[ind];
...
void StillingerWeberRadialBasis::CalcEFS(Configuration& cfg) {
    ...
    for (Neighborhood& nbh : cfg.nbhs) {
        ...
        //cout << "ind = " << ind << endl;
        AccumulateEFS2body(cfg, nbh, nbh.my_ind);
        AccumulateEFS3body(cfg, nbh, nbh.my_ind);
    }
...
Array3D& forces_grad, Array3D& stress_grad) {
    ...
    for (Neighborhood& nbh : cfg.nbhs) {
        ...
        //cout << "ind = " << ind << endl;
        AccumulateEFSGrad2body(cfg, nbh, nbh.my_ind, energy_grad, forces_grad, stress_grad);
        AccumulateEFSGrad3body(cfg, nbh, nbh.my_ind, energy_grad, forces_grad, stress_grad);
    }
}
...
StillingerWeberRadialBasis::StillingerWeberRadialBasis(const std::string& filename) 
{
    ...
    rad_bas_val_j.resize(p_RadialBasis->size);
    rad_bas_der_j.resize(p_RadialBasis->size);
}
...
void StillingerWeberRadialBasis::Load(const string& filename)
{
    ...
    else if (rbasis_type == "BChebyshev")
    ...
    //ifs >> foo_str >> foo_char >> min_val;
    //getline(ifs, line);
    //ifs >> foo_str >> foo_char >> max_val;
    //getline(ifs, line);
    //ifs >> foo_str >> foo_char >> radial_basis_size;
    //getline(ifs, line);

    int n_coeffs = 2*n_at_types*n_at_types*(p_RadialBasis->size+n_at_types)+n_at_types;
...
void StillingerWeberRadialBasis::Save(const string& filename)
{
    ...
    ofs << "    min_val = " << p_RadialBasis->min_val << endl;
    ofs << "    max_val = " << p_RadialBasis->max_val << endl;
    ofs << "    basis_size = " << p_RadialBasis->size << endl;
