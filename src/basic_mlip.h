...
protected:
    ...
    void InitMagMomBasis(Configuration& cfg);       // Initialization of magnetic moments basis and its derivatives

public:
    AnyBasis* p_RadialBasis = nullptr;              //!< pointer to RadialBasis
    AnyBasis* p_MagneticBasis = nullptr;            //!< pointer to MagneticBasis
    Array2D cfg_magnetic_basis_val;                 //!< magnetic basis for each magnetic moment in configuration
    Array2D cfg_magnetic_basis_der;                 //!< derivative of magnetic basis for each magnetic moment in configuration (w.r.t. magnetic moment)
    ...
    std::vector<double> buff_se_ders_wrt_magmom_neigh_;  //!< Temporal variable storing derivatives of site energy w.r.t. magnetic moments of neighboring atoms
    double buff_se_ders_wrt_magmom_central_;        //!< Temporal variable storing derivatives of site energy w.r.t. magnetic moments of central atom
...
public:
    ...
    virtual void CalcForcesGrad(Configuration& cfg, Array3D& out_frc_grad);                         // Must calculate gradient of the forces w.r.t. coefficients and write them in the third argument
    virtual void CalcStressesGrads(Configuration& cfg, Array3D& out_str_grad);                      // Must calculate gradient of the stress tensor w.r.t. coefficients and write them in the third argument. Order of indices in out_str_grad: (i,j,k) where i,j are the stress tensor components, k indicates derivative over k-th coefficient of MLIP
    virtual void CalcEFSGrads(  Configuration& cfg,                                                 // Must calculate gradients of the energy, all forces and stress tensor w.r.t. coefficients and write them in output parameters. Order of indices in out_frc_grad: (i,j,k) where i is atom index, j is force component (0,1,2), k indicates derivative over k-th coefficient of MLIP. Order of indices in out_str_grad is the same as in CalcStressesGrads(...)
...
//
public: 
    // functions calculating mindist, cutoff, minimal and maximum magnetic moments
    inline double CutOff();
    inline double MinDist();
    inline double MaxMagMom();
    inline double MinMagMom();

    // Constructors
    AnyLocalMLIP(double _min_val, double _max_val, int _radial_basis_size);
    AnyLocalMLIP(AnyBasis* _p_radial_basis, AnyBasis* _p_magnetic_basis = nullptr);
    AnyLocalMLIP() {}
...
inline double AnyLocalMLIP::MinDist()
{
    ...
    if (p_RadialBasis == nullptr)
        ERROR("Attempting to access uninitialized RadialBasis");
#endif
    return p_RadialBasis->min_val;
}

inline double AnyLocalMLIP::MaxMagMom()
{
#ifdef MLIP_DEBUG
    if (p_MagneticBasis == nullptr)
        ERROR("Attempting to access uninitialized MagneticBasis");
#endif 
    return p_MagneticBasis->max_val;
}

inline double AnyLocalMLIP::MinMagMom()
{
#ifdef MLIP_DEBUG
    if (p_MagneticBasis == nullptr)
        ERROR("Attempting to access uninitialized MagneticBasis");
#endif
    return p_MagneticBasis->min_val;
}
