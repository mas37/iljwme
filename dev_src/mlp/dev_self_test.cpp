...
TEST("MLIP_wrapper test1 (Recording cfgs, EFS via abinitio)")
{
	{ofstream ofs(PATH+"temp/Track1.cfg"); }
	
	Settings settings;
	settings["mlip"] = "mtp";
	settings["mlip:load-from"] = PATH+"MTP9.mtp";
	settings["write-cfgs"] = PATH+"temp/Track1.cfg";
	settings["select"] = "FALSE";
	settings["abinitio"] = "void";
	MLIP_Wrapper wrp(settings);

	CfgReader reader(PATH+"Li_T450.cfgs", &wrp);
	reader.Run();

	ifstream ifs1(PATH+"Li_T450.cfgs", ios::binary);
	ifstream ifs2(PATH+"temp/Track1.cfg", ios::binary);
...
TEST("MLIP_wrapper test2 (EFS via MTP, Recording cfgs, Error Monitoring)")
{
	{ofstream ofs(PATH+"temp/Track2.cfg"); }
	{ofstream ofs(PATH+"temp/delete_me.tmp"); }

	Settings settings;
	settings["mlip"] = "mtp";
	settings["mlip:load-from"] = PATH+"MTP100_3all.mtp";
	settings["calculate-efs"] = "TRUE";
	settings["write-cfgs"] = PATH+"temp/Track2.cfg";
	settings["check-errors"] = "temp/errors.txt";
	settings["check-errors:log"] = PATH+"temp/delete_me.tmp";
	settings["select"] = "FALSE";
	settings["abinitio"] = "void";
	...
	if (
		...
		(fabs(wrp.errmon.epa_aveabs() - 0.000231447) < 1.0e-9) ||
		(fabs(wrp.errmon.frc_aveabs() - 0.00531237) < 1.0e-8) ||
		(fabs(wrp.errmon.str_aveabs() - 0.316275) < 1.0e-6)
		)
		FAIL();
} END_TEST;

TEST("MLIP_wrapper test3 (Learning, Recording cfgs, EFS from ab-initio)")
{
	{ofstream ofs(PATH+"temp/Track3.cfg"); }
	{ofstream ofs(PATH+"temp/MTP9fitted.mtp"); }

	{
		...
		settings["mlip:load-from"] = PATH+"MTP9.mtp";
		settings["fit"] = "TRUE";
		settings["fit:save"] = PATH+"temp/MTP9fitted.mtp";
		settings["fit:energy-weight"] = "1.0";
		settings["fit:force-weight"] = "0.001";
		settings["fit:stress-weight"] = "0.1";
		settings["write-cfgs"] = PATH+"temp/Track3.cfg";
		settings["select"] = "FALSE";
		settings["abinitio"] = "void";
		...
		CfgReader reader(PATH+"Li_T450.cfgs", &wrp);
		reader.max_cfg_cnt = 20;
		reader.Run();
	}

	MTP mtp(PATH+"MTP9.mtp");
	LinearRegression lr(&mtp, 1, 0.001, 0.1);
	Configuration cfg;
	int cntr = 0;
	for (ifstream ifs(PATH+"Li_T450.cfgs", ios::binary); cfg.Load(ifs) && cntr<20; cntr++)
		lr.AddToSLAE(cfg);
	lr.Train();
	
	ifstream ifs(PATH+"Li_T450.cfgs", ios::binary);
	cfg.Load(ifs);
	cfg.Load(ifs);
	cfg.Load(ifs);
	Configuration cfg1(cfg), cfg2(cfg);
	mtp.CalcEFS(cfg1);
	MTP mtp2(PATH+"temp/MTP9fitted.mtp");
	mtp2.CalcEFS(cfg2);
...
TEST("MLIP_wrapper test4 (Learning + Selection, EFS from ab-initio)")
{
	{ofstream ofs(PATH+"temp/TS_9-1.cfg"); }
	{ofstream ofs(PATH+"temp/MTP9fitted.mtp"); }

	{
		...
		settings["fit:save"] = PATH+"temp/MTP9fitted.mtp";
		settings["fit:energy-weight"] = "1.0";
		settings["fit:force-weight"] = "1.0";
		settings["fit:stress-weight"] = "9.0";
		settings["select"] = "TRUE";
		settings["select:threshold-init"] = "0.000001";
		settings["select:threshold"] = "1.1";
		settings["select:threshold-swap"] = "1.000001";
		settings["select:site-en-weight"] = "0.0";
		settings["select:energy-weight"] = "1.0";
		settings["select:forces-weight"] = "0.0";
		settings["select:stress-weight"] = "0.0";
		settings["select:save-selected"] = PATH+"temp/TS_9-1.cfg";
		settings["abinitio"] = "void";
	...
	MTP mtp(PATH+"MTP9.mtp");
	MaxvolSelection mv(&mtp, 0.000001, 1.1, 1.000001);
	LinearRegression lr(&mtp, 1.0, 1.0, 9.0);
	Configuration cfg;
	int cntr = 0;
	for (ifstream ifs(PATH+"Li_T450.cfgs", ios::binary); cfg.Load(ifs); cntr++)
		mv.Select(cfg);
	
	mv.SaveSelected(PATH+"temp/TS_9-2.cfg");
	ifstream ifs1(PATH+"temp/TS_9-1.cfg", ios::binary);
	ifstream ifs2(PATH+"temp/TS_9-2.cfg", ios::binary);
	...
	if (cfg1.energy - cfg2.energy > 1.0e-10 || (cfg1.stresses - cfg2.stresses).MaxAbs() > 1.0e-10)
		FAIL();
} END_TEST;

TEST("MLIP_wrapper test5 (Loading selection + Selection)")
{
#ifdef GEN_TESTS
	MTP mtp(PATH+"MTP9.mtp");
	MaxvolSelection mv(&mtp, 0.000001, 1.1, 1.000001);
	Configuration cfg;
	int cntr = 0;
	for (ifstream ifs(PATH+"Li_T450.cfgs"); cfg.Load(ifs); cntr++)
		mv.Select(cfg);
	mv.SaveSelected(PATH+"TS61-1.cfgs");

	MTP mtp2(PATH+"MTP61.mtp");
	MaxvolSelection mv2(&mtp2, 0.000001, 1.1, 1.000001, 0, 1, 0, 0);
	mv2.LoadSelected(PATH+"TS61-1.cfgs");
	mv2.Save(PATH+"test.mvs");

	MaxvolSelection mv3(&mtp2);
	mv3.Load(PATH+"test.mvs");
	for (ifstream ifs(PATH+"Ti4MVtest.proc3"); cfg.Load(ifs); cntr++)
		mv2.Select(cfg);
	mv2.SaveSelected(PATH+"TS61-3.cfgs");
#endif
	
#ifdef MLIP_MPI
	int mpi_rank;
	MPI_Comm_rank(MPI_COMM_WORLD, &mpi_rank);
	{ ofstream ofs(PATH+"temp/TS61-1.cfg_"+to_string(mpi_rank)); }
#else
	{ ofstream ofs(PATH+"temp/TS61-1.cfg"); }
#endif

	{
		Settings settings;
		settings["mlip"] = "mtp";
		settings["mlip:load-from"] = PATH+"MTP61.mtp";
		settings["select"] = "TRUE";
		settings["select:load-state"] = PATH+"test.mvs";
		settings["select:save-selected"] = PATH+"temp/TS61-1.cfg";
		settings["select:threshold_init"] = "0.000001";
		settings["select:threshold"] = "1.1";
		settings["select:threshold-swap"] = "1.000001";
		settings["abinitio"] = "void";
		...
		CfgReader reader(PATH+"Ti4MVtest.cfg", &wrp);
		reader.Run();
	}

	ifstream ifs1(PATH+"TS61-3.cfgs", ios::binary);
#ifdef MLIP_MPI
	ifstream ifs2(PATH+"temp/TS61-1.cfg_"+to_string(mpi_rank), ios::binary);
#else
	ifstream ifs2(PATH+"temp/TS61-1.cfg", ios::binary);
#endif
	Configuration cfgg1, cfgg2;
	while (cfgg1.Load(ifs1))
	{
...
TEST("MLIP_wrapper test6 (Loading selection + LOTF, track errors)")
{
#ifdef MLIP_MPI
	int mpi_rank;
	MPI_Comm_rank(MPI_COMM_WORLD, &mpi_rank);
	{ofstream ofs(PATH+"temp/test2.mvs_"+to_string(mpi_rank)); }
	{ofstream ofs(PATH+"temp/MTP9lotf.mtp_"+to_string(mpi_rank)); }
	{ofstream ofs(PATH+"temp/delete_me.tmp_"+to_string(mpi_rank)); }
	{ofstream ofs(PATH+"temp/TS9lotf.cfg_"+to_string(mpi_rank)); }
#else
	{ofstream ofs(PATH+"temp/test2.mvs"); }
	{ofstream ofs(PATH+"temp/MTP9lotf.mtp"); }
	{ofstream ofs(PATH+"temp/delete_me.tmp"); }
	{ofstream ofs(PATH+"temp/TS9lotf.cfg"); }
#endif

	// Creating mvs-file for loading
	MTP mtp(PATH+"MTP9-3.mtp");
	MaxvolSelection mv(&mtp, 0.000001, 1.1, 1.000001);
	mv.LoadSelected(PATH+"TS9load.cfgs");
	mv.threshold_select = 2.0;
	mv.Save(PATH+"temp/test2.mvs");

#ifdef GEN_TESTS
	{
		Configuration cfg;
		int cntr = 0;
		for (ifstream ifs(PATH + "B.cfgs", ios::binary); cfg.Load(ifs); cntr++)
			mv.Select(cfg);
		mv.SaveSelected(PATH+"TS9lotf_check.cfgs");

		LinearRegression lr(&mtp, 1, 0, 0);
		lr.Train(mv.selected_cfgs);
		mtp.Save(PATH+"MTP9lotf_check.mtp");
	}
#endif
	
	{
		...
		settings["mlip"] = "mtp";
		settings["mlip:load-from"] = PATH+"MTP9-3.mtp";
		settings["calculate-efs"] = "TRUE";								// Enables/disables EFS calculation by MTP (disabled learning may be usefull in pure learning/selection regime for best performance)
		settings["fit"] = "TRUE";							// Enables/disables MTP learning
		settings["fit:save"] = PATH+"temp/MTP9lotf.mtp";			// Output MTP file name (for trained MTP)
		settings["fit:energy-weight"] = "1.0";						// Weight for energy equation in fitting procedure
		settings["fit:forces-weight"] = "0.0";						// Weight for forces equations in fitting procedure
		settings["fit:stress-weight"] = "0.0";						// Weight for stresses equations in fitting procedure
		settings["fit:scale-by-force"] = "0.0";						// Experimental setting. It is recommended to set it 0.0
		settings["select"] = "TRUE";							// Activates/deactivates selection (active learning engine)
		settings["select:threshold-init"] = "0.000001";				// Threshold for initiating selecting configurations to initial set. Configuration can not be selected if it change the Volume less than this times
		settings["select:threshold"] = "2";						// Selection threshold - maximum allwed extrapolation level
		settings["select:threshold-swap"] = "1.000001";				// Treshold for the following swaps if Selection threshold is reached
		settings["select:site-en-weight"] = "0.0";						// Weight for site energy equations in selection procedure
		settings["select:energy-weight"] = "1.0";						// Weight for energy equation in selection procedure
		settings["select:force-weight"] = "0.0";						// Weight for forces equations in selection procedure
		settings["select:stress-weight"] = "0.0";						// Weight for stresses equations in selection procedure
		settings["select:save-selected"] = PATH+"temp/TS9lotf.cfg";			// Selected configurations will be saved in this file after selection is complete. No configuration saved if not specified
		settings["select:load-state"] = PATH+"temp/test2.mvs";			// Filename for proliminary selection
		settings["check-errors"] = PATH + "temp/delete_me.tmp";							// Compares MLIP calculated EFS with Ab-initio model and writes errors for each configuration. Requires Ab-initio model specified
		settings["select:efs-ignored"] = "FALSE";						// FALSE - pass to driver QM data for EFS when learning occurs, TRUE - only MLIP-calculated data will be passed to driver
		settings["abinitio"] = "void";

		MLIP_Wrapper wrp(settings);

		CfgReader reader(PATH + "B.cfgs", &wrp);
		reader.Run();

		//cout << wrp.errmon.ene_rmsabs() << endl;
		//cout << wrp.errmon.frc_rmsabs() << endl;
		//cout << wrp.errmon.str_rmsabs() << endl;

		if (fabs(wrp.errmon.ene_rmsabs() - 9.30147) > 1.0e-5)
			FAIL();
		if (fabs(wrp.errmon.frc_rmsabs() - 4.67613) > 1.0e-5)
			FAIL();
		if (fabs(wrp.errmon.str_rmsabs() - 306.567) > 1.0e-3)
			FAIL();
	}

#ifdef MLIP_MPI
	ifstream ifs1(PATH+"temp/TS9lotf.cfg_0", ios::binary);
#else
	ifstream ifs1(PATH+"temp/TS9lotf.cfg", ios::binary);
#endif
	ifstream ifs2(PATH+"TS9lotf_check.cfgs", ios::binary);
	Configuration cfgg1, cfgg2;
	while (cfgg1.Load(ifs1))
	{
		cfgg2.Load(ifs2);
	...
	MTP mtp1(PATH+"temp/MTP9lotf.mtp");
	MTP mtp2(PATH+"MTP9lotf_check.mtp");
	ifstream ifss(PATH+"4Validation.cfg", ios::binary);
	cfgg1.Load(ifss);
	cfgg2 = cfgg1;
	mtp1.CalcEFS(cfgg1);
	mtp2.CalcEFS(cfgg2);
	if (fabs(cfgg1.energy - cfgg2.energy) > 1.0e-7)
		FAIL();
	if ((cfgg1.stresses - cfgg2.stresses).MaxAbs() > 1.0e-7)
		FAIL();
	
} END_TEST;

TEST("MLIP_wrapper test7 (Breaking treshold test)")
{
#ifdef  MLIP_MPI
	int mpi_rank;
	MPI_Comm_rank(MPI_COMM_WORLD, &mpi_rank);
	{ofstream ofs(PATH + "temp/TS_break.cfgs_" + to_string(mpi_rank)); }
#else
	{ofstream ofs(PATH + "temp/TS_break.cfgs"); }
#endif //  MLIP_MPI
	{ofstream ofs(PATH + "temp/test2.mvs"); }
	{ofstream ofs(PATH + "temp/MTP9lotf.mtp"); }
	{ofstream ofs(PATH + "temp/delete_me.tmp"); }
...

	{
		...
		settings["calculate-efs"] = "TRUE";								// Enables/disables EFS calculation by MTP (disabled learning may be usefull in pure learning/selection regime for best performance)
		settings["select"] = "TRUE";										// Activates/deactivates selection (active learning engine)
		settings["select:threshold"] = "4.0";							// Selection threshold - maximum allwed extrapolation level
		settings["select:threshold-break"] = "8.0";					// Selection threshold - maximum allwed extrapolation level
		settings["select:site-en-weight"] = "0.0";						// Weight for site energy equations in selection procedure
		settings["select:energy-weight"] = "1.0";						// Weight for energy equation in selection procedure
		settings["select:force-weight"] = "0.0";						// Weight for forces equations in selection procedure
		settings["select:stress-weight"] = "0.0";						// Weight for stresses equations in selection procedure
		settings["select:save-selected"] = PATH + "temp/TS_break.cfgs";			// Selected configurations will be saved in this file after selection is complete. No configuration saved if not specified
		settings["select:load-state"] = PATH + "temp/test2.mvs";			// Filename for proliminary selection
		settings["abinitio"] = "void";

		MLIP_Wrapper wrp(settings);

		CfgReader reader(PATH + "B_alter.cfgs", &wrp);
		try
		{
			reader.Run();
		}
		...
		{
			if (excp.What() != (string)"Breaking threshold exceeded (MV-grade: 8.268346)")
			{
				cout << endl << excp.What() << "|" << endl;
				cout << "Breaking threshold exceeded (MV-grade: 8.268346)" << "|" << endl;
				FAIL();
			}
		}

#ifdef  MLIP_MPI
		auto cfgs = LoadCfgs(PATH + "temp/TS_break.cfgs_" + to_string(mpi_rank));
#else
		auto cfgs = LoadCfgs(PATH + "temp/TS_break.cfgs");
#endif //  MLIP_MPI
		if (cfgs.size() != 35)
			FAIL();
		for (int i=0; i<cfgs.size(); i++)
		{
			if (stod(cfgs[i].features["MV_grade"]) < stod(settings["select:threshold"]))
				FAIL();
			if (stod(cfgs[i].features["MV_grade"]) > stod(settings["select:threshold-break"]) &&  i != cfgs.size()-1)
				FAIL();
		}
	}
